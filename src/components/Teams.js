...
function Team_Content() {
  return (
    <div className="flex items-start  lg:items-center flex-col px-4 gap-y-2 lg:justify-center">
      ...
      <h1 className="lg:text-4xl text-3xl font-Inter text-center font-bold mb-4 lg:mb-6 tracking-wider text-white leading-10">
        The Team
      </h1>
      <p className="text-base font-Poppins w-full mb-6 text-cente tracking-wide text-white">
        We are a global team of experienced and personable developers, who are
        looking to use blockchain to bring equality and transformation to the
        world.
      </p>
      <button className="bg-[#BE173F] hover:opacity-80 lg:w-[60%] lg:mb-6 text-white text-sm lg:text-lg font-Poppins px-6 lg:px-10 py-4 font-bold lg:py-4 rounded-lg">
        <a href=" https://www.linkedin.com/in/djrthree/" target="_blank">
          Know more about our team{" "}
        </a>
      </button>
      <div className="w-[100%] justify-start mt-8 items-start flex py-2"></div>
    </div>
  );
}
      ...
      >
        ...
        >
          <img
            src={img}
            alt="image"
            className="bg-cover flex w-[70%] lg:w-[75%]"
          />
          <div className="text-left lg:text-right py-2 text-lg md:text-xl font-Poppins">
            ...
            <span className="text-[#BE173F] tracking-widest font-bold">
              CEO
            </span>
          </div>
        </div>
        <div className="basis-[80%] items-start justify-start  text-left">
          {content}
        </div>
      </div>
    </div>
...
function Teams() {
  return (
    <div className="lg:mb-20 dark_top pt-10" id="team">
      <div
        className="lg:container bg-no-repeat w-[95%]  mx-auto"
